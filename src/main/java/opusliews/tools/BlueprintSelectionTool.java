...
public class BlueprintSelectionTool implements GameTool {
	...
	private void addSelectionDrawable(List list, GameCamera camera) {
		...
		SharedTextureDrawOptions drawOptions = Zoning.getRectangleDrawOptions(
				drawRectangle, edgeColor, fillColor, camera);
	...
	private void addExcludedTileDrawables(List list, GameCamera camera) {
		for (Point excludedTile : excludedTiles) {
			...
			Rectangle drawRectangle = new Rectangle(tile.x * 32, tile.y * 32, 32, 32);
			Color edgeColor = new Color(255, 0, 0, 220);
			Color fillColor = new Color(0, 0, 0, 0);
			SharedTextureDrawOptions tileDrawOptions = Zoning.getRectangleDrawOptions(
					drawRectangle, edgeColor, fillColor, camera);
	...
	private void drawExcludedX(GameCamera camera, Point tile) {
		...
		xIcon.initDraw().size(32, 32).pos(drawX, drawY).draw();
	}
	...
	private Rectangle getCurrentSelection(GameCamera camera) {
		...
		return makeRectangle(mouseDownTile.x, mouseDownTile.y, tileX, tileY);
	}
	...
	private boolean isCtrlDown() {
		return WindowManager.getWindow().getInput().isKeyDown(GLFW.GLFW_KEY_LEFT_CONTROL)
	...
	private void toggleExcludedTile(int tileX, int tileY) {
		if (selection == null || !selection.contains(tileX, tileY)) {
			return;
		}
	...
	public boolean inputEvent(InputEvent event) {
		...
		if (!event.isUsed() && event.state && event.getID() == -100) {
			if (isCtrlDown()) {
				if (selection != null && selection.contains(tileX, tileY)) {
					ctrlMouseDownTile = new Point(tileX, tileY);
					event.use();
					return true;
				}
				return false;
		...
		if (!event.state && event.getID() == -100) {
			if (ctrlMouseDownTile != null) {
				if (ctrlMouseDownTile.x == tileX && ctrlMouseDownTile.y == tileY) {
			...
			if (mouseDownTile != null) {
				selection = makeRectangle(mouseDownTile.x, mouseDownTile.y, tileX, tileY);
				mouseDownTile = null;
				excludedTiles.clear();
				event.use();
				return true;
	...
	public List<String> getExcludedObjectIDs() {
		...
		for (Point tile : excludedTiles) {
			for (Integer layerID : ObjectLayerRegistry.getLayerIDs()) {
				GameObject object = level.getObject(layerID, tile.x, tile.y);
				if (object == null) {
					continue;
				}
	...
	public GameTooltips getTooltips() {
		...
		tooltips.add(new InputTooltip(
				-100,
				Localization.translate("ui", "blueprintselectarea")
		));
		return tooltips;
	}
}
