...
public class PlacedLogObject extends GameObject {
	...
	private final String plankStringID;
	...
	public LootTable getLootTable(Level level, int layerID, int tileX, int tileY) {
		return new LootTable(new LootItemInterface[]{
				new LootItem(sourceLogStringID, 1)
		});
	}
	...
	public void interact(Level level, int tileX, int tileY, PlayerMob player) {
		InventoryItem selectedItem = player.getSelectedItem();
		if (selectedItem == null || !(selectedItem.item instanceof AxeToolItem)) {
			return;
		}
		...
		InventoryItem planks = new InventoryItem(plankStringID, 4);
		level.entityManager.pickups.add(
				planks.getPickupEntity(
						level,
						tileX * 32.0F + 16.0F,
						tileY * 32.0F + 16.0F
				)
